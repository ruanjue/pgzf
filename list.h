...
#define list4array(array_macro, list, expr) array_macro((list)->buffer, (list)->size, typeof((list)->buffer[0]), expr)
#define sort_list(list, cmpgt_expr) list4array(sort_array, list, cmpgt_expr)
...
static inline e_type* head_##list_type(list_type *list){	\
	if(list->cap) return list->buffer;	\
	else return NULL;	\
}	\
	\
...
static inline void append_array_##list_type(list_type *list1, e_type *ary, size_type size){	\
	...
	\
static inline void append_zeros_##list_type(list_type *list, size_type size){	\
	if(size == 0) return;	\
	encap_##list_type(list, size);	\
	memset(list->buffer + list->size, 0, size * sizeof(e_type));	\
	list->size += size;	\
}	\
	\
