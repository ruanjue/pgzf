...
#define cvt2x32(a, b) ((((u8i)(a)) << 32) | (b))
#define cvt4x16(a, b, c, d) ((((u8i)(a)) << 48) | (((u8i)(b)) << 32) | (((u8i)(c)) << 16) | (d))
...
#define swap_bytes(_ap, _bp, _n) {	\
	...
	bp = (void*)(_bp);	\
	for(i=0;i+8<=n;i+=8){	\
		t = ((u8i*)ap)[i >> 3]; ((u8i*)ap)[i >> 3] = ((u8i*)bp)[i >> 3]; ((u8i*)bp)[i >> 3] = t;	\
	}	\
	for(;i+4<=n;i+=4){	\
		t = ((u4i*)ap)[i >> 2]; ((u4i*)ap)[i >> 2] = ((u4i*)bp)[i >> 2]; ((u4i*)bp)[i >> 2] = t;	\
	}	\
	for(;i+2<=n;i+=2){	\
		t = ((u2i*)ap)[i >> 1]; ((u2i*)ap)[i >> 1] = ((u2i*)bp)[i >> 1]; ((u2i*)bp)[i >> 1] = t;	\
	}	\
	for(;i<n;i++){	\
		t = ((u1i*)ap)[i]; ((u1i*)ap)[i] = ((u1i*)bp)[i]; ((u1i*)bp)[i] = t;	\
	}	\
}
...
//#pragma message("LITTLE_ENDAIN")
#define BIT64_TO_BIG_ENDIAN(V) __builtin_bswap64(V)
#define BIT32_TO_BIG_ENDIAN(V) __builtin_bswap32(V)
#define BIT16_TO_BIG_ENDIAN(V) __builtin_bswap16(V)
#define BIT64_TO_LITTLE_ENDIAN(V) (V)
#define BIT32_TO_LITTLE_ENDIAN(V) (V)
#define BIT16_TO_LITTLE_ENDIAN(V) (V)
#else
//#pragma message("BIG_ENDAIN")
#define BIT64_TO_BIG_ENDIAN(V) (V)
#define BIT32_TO_BIG_ENDIAN(V) (V)
#define BIT16_TO_BIG_ENDIAN(V) (V)
#define BIT64_TO_LITTLE_ENDIAN(V) __builtin_bswap64(V)
#define BIT32_TO_LITTLE_ENDIAN(V) __builtin_bswap32(V)
#define BIT16_TO_LITTLE_ENDIAN(V) __builtin_bswap16(V)
#endif
...
#define fprintf_msg(out, ...) { fprintf(out, "\e[31;44m"); fprintf(out, __VA_ARGS__); fprintf(out, "\e[0m"); fflush(out); }
#define fprintf_err(out, ...) { fprintf(out, "\e[7;31;44m"); fprintf(out, __VA_ARGS__); fprintf(out, "\e[0m"); fflush(out); }

#define WARNONCE(str)	\
BEG_RUNONCE;	\
fflush(stdout); fprintf_err(stderr, " -- \"%s\" in %s -- %s:%d --\n", (str), __FUNCTION__, __FILE__, __LINE__); fflush(stderr);	\
END_RUNONCE
...
static int has_debugger = -1;

static inline void _sigtrap_handler(int signum){
	UNUSED(signum);
	has_debugger = 0;
...
static inline void vertical_print_array_u4i(FILE *out, int nrow, u4i *ptr, int len){
	u4i val;
	int i, j, c;
	char ch;
	for(i=nrow-1;i>=0;i--){
